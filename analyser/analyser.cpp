...
namespace miniplc0 {
    ...
    std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
        ...
        while(true) {
            ...
            if (isDeclared(next.value().GetValueString()))
                return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrDuplicateDeclaration);
            //addVariable(next.value());//这里add了就是声明了
    ...
    std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
        ...
        if(next.value().GetType()==TokenType::MINUS_SIGN){
            ...
            if(next.value().GetType()!=TokenType::UNSIGNED_INTEGER)
                return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrConstantNeedValue);
            auto value=next.value().GetValue();
            int32_t unsigned_value=std::any_cast<int32_t >(value);
            if(unsigned_value-1>INT32_MAX)
                return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrAssignToConstant);//溢出
            out = (-1)*(int32_t)unsigned_value;
    ...
    std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
        ...
        if(!next.has_value()||next.value().GetType()!=TokenType::EQUAL_SIGN)
            return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrInvalidAssignment);
        _instructions.emplace_back(Operation::LIT, 0);//防止栈空间被覆盖
        _instructions.emplace_back(Operation::LIT, 0);
        err=analyseExpression();//表达式结果存在了栈顶
        if(err.has_value())
            return err;
        next = nextToken();
        if(next.value().GetType() != TokenType::SEMICOLON)
            return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrInvalidAssignment);
        if(!isInitializedVariable(iden.value().GetValueString()))
            addVariable(iden.value());
        int32_t x=_vars[iden.value().GetValueString()];
        _instructions.emplace_back(Operation::STO, x);
        return {};
    }
    ...
    std::optional<CompilationError> Analyser::analyseFactor() {
        ...
        switch (next.value().GetType()) {
            ...
            case TokenType ::IDENTIFIER:{
                ...
                int32_t x;
                if(_vars.find(next.value().GetValueString()) != _vars.end())
                    x= _vars[next.value().GetValueString()];
                else
                    x=_consts[next.value().GetValueString()];
                _instructions.emplace_back(Operation::LOD, x);
                _instructions.emplace_back(Operation::LIT, prefix);
                _instructions.emplace_back(Operation::MUL, 0);
                break;}
            case TokenType ::UNSIGNED_INTEGER:{
                next=nextToken();
                int32_t unsigned_value=std::any_cast<int32_t >(next.value().GetValue());
                if(unsigned_value-1>INT32_MAX)
                    return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrAssignToConstant);//溢出
                _instructions.emplace_back(Operation::LIT, prefix);
                _instructions.emplace_back(Operation::LIT, unsigned_value);
                _instructions.emplace_back(Operation::MUL, 0);
                break;}
